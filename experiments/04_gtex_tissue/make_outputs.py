"""Build GTEx tissue outputs from logged results."""
...
RESULTS = ROOT / "results" / "gtex_tissue" / "results.json"
...
def main() -> None:
    ...
    cols = ["Method", "Balanced accuracy", "Macro-F1", "MCC", "AUROC OvR", "AUPRC OvR"]
    df[cols].to_csv(TABLE_DIR / "table_gtex_500g.csv", index=False)
    (TABLE_DIR / "table_gtex_500g.md").write_text(_to_markdown(df[cols]))
