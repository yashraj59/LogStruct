...
def get_coefficients(model, effective=True):
    """
    Return model coefficients for interpretation.

    By default this returns ``effective_coef_``, which folds the smoothing
    operator into the linear head and is therefore the coefficient vector of
    the equivalent linear model on the original feature space.
    """
    if effective and hasattr(model, "effective_coef_"):
        return model.effective_coef_
    if hasattr(model, "coef_"):
        return model.coef_
    raise ValueError("Model does not expose coefficients. Fit the model first.")


def top_coefficients(model, gene_names=None, n=20, effective=True, class_index=None):
    """
    Return the top coefficients by absolute value.

    Args:
        model: Trained LogStructClassifier or LogStructRegressor.
        gene_names: Optional ordered gene names.
        n: Number of coefficients to return.
        effective: Prefer smoothing-adjusted coefficients when available.
        class_index: Optional row index for multiclass/multitarget models.

    Returns:
        List of (index, name, coefficient, abs_coefficient), sorted descending.
    """
    coef = np.asarray(get_coefficients(model, effective=effective))
    if coef.ndim > 1:
    ...
    if gene_names is None:
        gene_names = [f"Feature_{i}" for i in range(coef.shape[0])]

    top_idx = np.argsort(np.abs(coef))[-n:][::-1]
    return [(int(i), str(gene_names[i]), float(coef[i]), float(abs(coef[i]))) for i in top_idx]
