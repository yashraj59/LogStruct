...
def from_pathway_gmt(
    ...
    with _open_text(gmt_path) as f:
        for line in f:
            ...
            genes_in_pathway = [
                gene_to_idx[g.upper()] for g in parts[2:] if g.upper() in gene_to_idx
            ]
...
def from_string_db(
    ...
    alias_path : str or Path, optional
        STRING aliases file (e.g., 9606.protein.aliases.v12.0.txt.gz). Provide
        this when ``gene_names`` are gene symbols rather than STRING protein
        identifiers.
    ...
    if alias_path is not None:
        with _open_text(alias_path) as f:
            next(f, None)
            for line in f:
                parts = line.rstrip("\n").split("\t")
                if len(parts) < 2:
                ...
                protein_id, alias = parts[0].upper(), parts[1].upper()
                if alias in gene_to_idx:
                    protein_to_idx[protein_id] = gene_to_idx[alias]

    adj = np.full((n, n), baseline, dtype=np.float32)

    string_path = Path(string_path)
    with _open_text(string_path) as f:
        ...
        for line in f:
            ...
            # Resolve either full STRING protein IDs via aliases or direct IDs.
            p1_key, p2_key = p1.upper(), p2.upper()
            g1 = p1.split(".")[-1].upper() if "." in p1 else p1.upper()
            g2 = p2.split(".")[-1].upper() if "." in p2 else p2.upper()
            i = protein_to_idx.get(p1_key, gene_to_idx.get(g1, gene_to_idx.get(p1_key)))
            j = protein_to_idx.get(p2_key, gene_to_idx.get(g2, gene_to_idx.get(p2_key)))
