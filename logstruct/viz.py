...
def plot_network_graph(
    ...
    nx.draw_networkx_edges(G, pos, ax=ax, width=edge_weights, alpha=0.6, edge_color="gray")
...
def plot_coefficients(
    ...
    coef : ndarray of shape (n_features,) or (n_classes, n_features)
        Coefficient vector(s), or a fitted LogStruct model. If a model is
        supplied and effective=True, ``effective_coef_`` is plotted when
        available.
    feature_names : sequence of str
        Names for features.
    top_n : int
    ...
    effective : bool
        Prefer smoothing-adjusted model coefficients when ``coef`` is a fitted
        LogStruct estimator.
    ...
    if hasattr(coef, "effective_coef_") or hasattr(coef, "coef_"):
        if effective and hasattr(coef, "effective_coef_"):
            coef = coef.effective_coef_
        else:
            coef = coef.coef_
